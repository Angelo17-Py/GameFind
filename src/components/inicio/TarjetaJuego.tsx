                    ...
                    <div className="space-y-2 mb-6 flex-grow">
                        {game.deals.length > 0 ? (
                            ...
                            <div className="p-4 rounded-2xl border border-white/10 bg-white/5 text-center">
                                <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Sin oferta actual</p>
                            </div>
                        )}
                    </div>

                    <button
                        onClick={() => bestDeal && window.open(bestDeal.url, '_blank')}
                        disabled={!bestDeal}
                        className="w-full bg-gradient-to-r from-purple-600 to-blue-600 enabled:hover:scale-[1.02] active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed px-6 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-lg shadow-purple-500/20"
                    >
                        {bestDeal ? 'IR A LA MEJOR OFERTA' : 'SIN OFERTA DISPONIBLE'}
                    </button>
                </div>
            </div>
        </div>
